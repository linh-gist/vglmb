...
%         %currsoln(var)= sample(tempsamp,1);
...
%do in real domain - faster method 
assignments(1,:)= currsoln;
costs(1)=sum(P0([1:n1]+(currsoln-1)*n1));
expnegP0tempd= exp(-P0_tempd);
for sol= 2:m
    for var= 1:n1
        tempsamp= expnegP0tempd(var,:); %grab row of costs for current association variable
        tempsamp(currsoln)= 0; tempsamp(currsoln(var))= expnegP0tempd(var,currsoln(var)); %lock out current and previous iteration step assignments except for the one in question

        cdf= cumsum(tempsamp);
        currsoln(var) = sum(cdf < rand*cdf(end)) + 1; 
    end
    assignments(sol,:)= currsoln;
    costs(sol)= sum(P0([1:n1]+(currsoln-1)*n1));
end
